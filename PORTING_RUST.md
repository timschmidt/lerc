...
- Added the first legacy Lerc1 metadata reader in `rust/src/lerc1.rs`, covering `CntZImage` header and part-header parsing, non-tiled count/mask decoding, and z-tile min/max statistics for the checked-in `world.lerc1` fixture.
...
- Added the Rust C ABI entry point `lerc_getDataRanges`, backed by the safe Lerc2 data-range aggregator, cross-checked against checked-in Lerc2 fixtures, and guarded with a panic boundary.
...
- The Rust Lerc1 reader currently validates the legacy `CntZImage` file key, version/type fields, dimensions, max-z-error, and count/z part headers. It also decodes non-tiled count parts stored as constant counts or RLE-compressed packed masks, and computes z min/max statistics from tiled z payloads. `get_lerc_info` and C ABI metadata functions remain Lerc2-only until this Lerc1 metadata is integrated into the public info path.
...
3. Add C++ parity tests for `lerc_getBlobInfo`.
   ...
   - Integrate Lerc1 count-mask and z-stat metadata into `get_lerc_info`, `lerc_getBlobInfo`, and `lerc_getDataRanges` for `world.lerc1`.
...
- Lerc1 C API metadata parity still needs integration of the decoded legacy metadata into the public info/range APIs.
