...
- Added encode-side C ABI support for constant single-band inputs through `lerc_computeCompressedSize`, `lerc_computeCompressedSizeForVersion`, `lerc_encode`, and `lerc_encodeForVersion`; non-constant inputs and 4D encode calls still return `Failed` until the broader Rust encoder is ported.
...
- Encode-side C ABI functions now route constant single-band inputs through the safe Rust constant encoder. Non-constant inputs and 4D encode calls intentionally fail with `ErrCode::Failed` after C++-style argument validation and output-counter zeroing.
