...
- Added the first legacy Lerc1 metadata reader in `rust/src/lerc1.rs`, covering `CntZImage` header and part-header parsing plus non-tiled count/mask decoding for the checked-in `world.lerc1` fixture.
...
- The Rust Lerc1 reader currently validates the legacy `CntZImage` file key, version/type fields, dimensions, max-z-error, and count/z part headers. It also decodes non-tiled count parts stored as constant counts or RLE-compressed packed masks. It does not yet decode Lerc1 z tile payloads, so `get_lerc_info` and C ABI metadata functions remain Lerc2-only for exact min/max reporting.
...
3. Add C++ parity tests for `lerc_getBlobInfo`.
   ...
   - Port Lerc1 z tile decoding to let `get_lerc_info`, `lerc_getBlobInfo`, and `lerc_getDataRanges` report exact min/max metadata for `world.lerc1`.
...
- Lerc1 C API metadata parity still needs legacy z tile decoding to compute min/max ranges.
