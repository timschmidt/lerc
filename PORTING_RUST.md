...
- Added `encode_lerc2_byte_huffman`, a safe single-band version 4+ byte Huffman encoder for `UChar` and `Char` data with `max_z_error == 0.5`. It writes the same Lerc2 Huffman payload envelope consumed by the supported decoder and selects the smaller regular-vs-delta Huffman candidate.
...
- Added safe Lerc2 encode range computation from typed little-endian source bytes, a version 4+ single-band one-sweep encoder, `encode_lerc2_one_sweep`, a version 4+ single-band raw tiled encoder, `encode_lerc2_tiled_raw`, a version 4+ single-band byte Huffman encoder, `encode_lerc2_byte_huffman`, concatenated multi-band one-sweep and raw tiled encoders, `encode_lerc2_one_sweep_bands` and `encode_lerc2_tiled_raw_bands`, v6 no-data sentinel handling via single-band and band-major helpers, and high-level `encode_lerc2_uncompressed` / `encode_lerc2_uncompressed_with_no_data` fallback selectors for uncompressed blobs. Raw tiled encoding emits image mode 0 when the header enters the C++ Huffman-probe envelope.
...
- Added encode-side C ABI support for constant, one-sweep single-band, one-sweep multi-band, and 4D calls with or without active no-data metadata through the same safe uncompressed selector used by the Rust API. Active no-data support expects the source data to contain the declared sentinel, masks all-depth no-data pixels, and remaps conflicting mixed-depth sentinels to an internal value for storage.
...
- Encode-side C ABI functions now route constant, one-sweep single-band, one-sweep multi-band, and 4D inputs with or without active no-data metadata through the safe Rust uncompressed selectors. Active no-data encode requires the caller's data to already contain the declared no-data sentinel and applies the same all-depth mask filtering and mixed-depth sentinel remapping as the safe helper.
...
6. Add FFI surface.
   ...
   - Constant, one-sweep single-band, one-sweep multi-band, and 4D C ABI encode paths now use the safe Rust encoder for no-data-free data and active no-data sentinels.
...
- `cargo test`: passed, 171 unit tests.
- `cargo doc --no-deps`: passed and generated crate documentation.
- `cargo bench --bench primitives`: passed and printed timings for RLE compress/decompress, BitStuffer encode/decode, pre-v2.3 BitStuffer encode/decode, BitMask conversion/count operations, typed decoded value conversion, checked decoded output copying, decoded `f64` conversion through C ABI, DecodeIntoSpec byte-count helpers, EncodeSpec validation and byte-count helpers, Lerc1 header parsing, Lerc1 count-mask reading, Lerc1 z-stat reading, Lerc1 float value decoding, Lerc1 safe decode-into, Lerc1 safe decode-to-`f64`, Lerc1 C ABI decode, Lerc1 C ABI decode-to-double, Lerc1 info aggregation, Lerc1 data-range aggregation, Lerc2 metadata parsing, C API-style blob info array filling, Lerc2 data-range aggregation, C ABI data-range retrieval, Lerc2 mask reading, Lerc2 checksum validation and finalization, Lerc2 header writing, Lerc2 mask writing, Lerc2 min/max range parsing and writing, Lerc2 one-sweep and raw tiled payload writing, Lerc2 constant encoding, high-level uncompressed encode selection with and without no-data metadata, Lerc2 one-sweep single-band, raw tiled single-band including image-mode-0 envelope, byte Huffman single-band, one-sweep and raw tiled multi-band, v6 one-sweep and raw tiled no-data metadata encoding, and v6 no-data metadata encoding, C ABI constant and one-sweep single-band, multi-band, no-active-no-data 4D, and active-no-data metadata 4D encode compute-size and encode, Lerc2 no-data metadata reporting, Lerc2 one-sweep decode, Lerc2 raw tiled decode, Lerc2 simple bit-stuffed tiled decode, Lerc2 LUT tiled decode, Lerc2 integer and floating-point diff tiled simple/LUT decode, high-level supported-subset Lerc2 decode, high-level supported-subset multi-band Lerc2 decode, Lerc2 safe decode-to-`f64`, real float fixture decode, byte Huffman fixture decode, v6 supported-subset no-data decode, C API-style supported decode-into-buffer validation, C ABI decode, C ABI decode-to-double, C ABI real float fixture decode, C ABI byte Huffman fixture decode, and C ABI 4D no-data decode variants.
...
- Floating-point Huffman, predictor-backed modes, multi-band byte Huffman encode selection, and C API compressed encode selection still need separate cross-language fixtures before being treated as parity-complete.
