...
- Added safe Lerc2 encode range computation from typed little-endian source bytes and a version 4+ single-band one-sweep encoder, `encode_lerc2_one_sweep`, for uncompressed fallback blobs.
...
- Added encode-side C ABI support for constant and one-sweep single-band inputs through `lerc_computeCompressedSize`, `lerc_computeCompressedSizeForVersion`, `lerc_encode`, and `lerc_encodeForVersion`; multi-band inputs and 4D encode calls still return `Failed` until the broader Rust encoder is ported.
...
- Encode-side C ABI functions now route constant and one-sweep single-band inputs through safe Rust encoders. Multi-band inputs and 4D encode calls intentionally fail with `ErrCode::Failed` after C++-style argument validation and output-counter zeroing.
...
- Version 4+ one-sweep Lerc2 encoding is now available as the first safe non-constant encode path and round-trips through the supported decoder.
...
6. Add FFI surface.
   ...
   - Constant and one-sweep single-band 2D C ABI encode paths now use the safe Rust encoder.
