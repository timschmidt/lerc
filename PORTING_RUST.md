...
- Floating-point Huffman predictor groundwork is staged internally: the Rust port now mirrors the C++ `Predictor` code/type/delta mapping and the byte-plane delta restoration used by `LosslessFPCompression::DecodeHuffmanFltSlice` before byte-order restoration. The full compressed byte-plane extraction and float/double reassembly path is still pending.
...
4. Port decode-only Lerc2 block paths.
   ...
   - Floating-point Huffman predictor code mapping and byte-plane delta restoration are now pinned in Rust tests.
