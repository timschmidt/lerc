...
- Added safe Lerc2 encode range computation from typed little-endian source bytes, a version 4+ single-band one-sweep encoder, `encode_lerc2_one_sweep`, and a concatenated multi-band one-sweep encoder, `encode_lerc2_one_sweep_bands`, for uncompressed fallback blobs.
...
- Added encode-side C ABI support for constant, one-sweep single-band, one-sweep multi-band, and 4D calls without active no-data bands through the Rust one-sweep encoder; 4D calls with active no-data still return `Failed` until no-data encode handling is ported.
...
- Encode-side C ABI functions now route constant, one-sweep single-band, one-sweep multi-band, and 4D inputs without active no-data through safe Rust encoders. 4D encode calls with active no-data intentionally fail with `ErrCode::Failed` after C++-style argument validation and output-counter zeroing.
...
6. Add FFI surface.
   ...
   - Constant, one-sweep single-band, one-sweep multi-band, and no-active-no-data 4D C ABI encode paths now use the safe Rust encoder.
