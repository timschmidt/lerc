...
- Floating-point Huffman predictor groundwork is staged internally: the Rust port now mirrors the C++ `Predictor` code/type/delta mapping, byte-plane delta restoration, byte-plane reassembly, float transform undo, and row/cross predictor restoration used by `LosslessFPCompression::DecodeHuffmanFltSlice`. The full compressed byte-plane extraction path is still pending.
...
4. Port decode-only Lerc2 block paths.
   ...
   - Floating-point Huffman predictor code mapping, byte-plane delta restoration, byte-plane reassembly, and row/cross predictor restoration are now pinned in Rust tests.
