...
- Added encode-side C ABI support for constant, one-sweep single-band, and one-sweep multi-band inputs through `lerc_computeCompressedSize`, `lerc_computeCompressedSizeForVersion`, `lerc_encode`, and `lerc_encodeForVersion`; 4D encode calls still return `Failed` until no-data encode handling is ported.
...
- Encode-side C ABI functions now route constant, one-sweep single-band, and one-sweep multi-band inputs through safe Rust encoders. 4D encode calls intentionally fail with `ErrCode::Failed` after C++-style argument validation and output-counter zeroing.
...
6. Add FFI surface.
   ...
   - Constant, one-sweep single-band, and one-sweep multi-band 2D C ABI encode paths now use the safe Rust encoder.
