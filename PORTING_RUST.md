...
- Added the first legacy Lerc1 metadata reader in `rust/src/lerc1.rs`, covering `CntZImage` header and part-header parsing, non-tiled count/mask decoding, z-tile min/max statistics, and safe `get_lerc_info` metadata fallback for the checked-in `world.lerc1` fixture.
...
- The Rust Lerc1 reader currently validates the legacy `CntZImage` file key, version/type fields, dimensions, max-z-error, and count/z part headers. It also decodes non-tiled count parts stored as constant counts or RLE-compressed packed masks, computes z min/max statistics from tiled z payloads, and is integrated into the safe `get_lerc_info` and blob-info array helper paths. `lerc_getBlobInfo` inherits that fallback through the shared helper.
...
3. Add C++ parity tests for `lerc_getBlobInfo`.
   ...
   - Integrate Lerc1 z-stat metadata into `get_lerc2_data_ranges` and `lerc_getDataRanges` for `world.lerc1`.
...
- Lerc1 C API data-range parity still needs integration of the decoded legacy z statistics into `get_lerc2_data_ranges`.
