...
- Added safe Lerc2 encode range computation from typed little-endian source bytes, a version 4+ single-band one-sweep encoder, `encode_lerc2_one_sweep`, a version 4+ single-band raw tiled encoder, `encode_lerc2_tiled_raw`, version 4+ single-band and band-major byte Huffman encoders with and without v6 no-data metadata, concatenated multi-band one-sweep and raw tiled encoders, `encode_lerc2_one_sweep_bands` and `encode_lerc2_tiled_raw_bands`, v6 no-data sentinel handling via single-band and band-major helpers, high-level `encode_lerc2_uncompressed` / `encode_lerc2_uncompressed_with_no_data` fallback selectors for uncompressed blobs, `encode_lerc2_auto` for no-data-free size-based encode choice, and `encode_lerc2_auto_with_no_data` for active no-data-aware size-based encode choice. Raw tiled encoding emits image mode 0 when the header enters the C++ Huffman-probe envelope. Integer one-sweep and raw tiled encoders, including single-band no-data wrappers, now accept C++-style negative `max_z_error` as a bit-plane epsilon and store the inferred nonnegative max error when enough statistics are available. Float and double one-sweep/raw tiled encoders now port the C++ positive-error raise heuristic when exact rounded candidates preserve the requested tolerance.
...
- Integer one-sweep and raw tiled encoders, including version 6 single-band no-data helpers, port the C++ negative-`maxZError` bit-plane heuristic: for integer data, a negative value is interpreted as an epsilon, noisy low bit planes may raise the stored max error, and the fallback stored error is `0.5` when the heuristic has too little data. Float and double inputs still reject negative values.
...
- `cargo test`: passed, 194 unit tests.
