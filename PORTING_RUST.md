...
- Added encode-side C ABI support for constant, one-sweep single-band, one-sweep multi-band, and 4D calls with or without active no-data metadata through the Rust one-sweep encoder. Active no-data support currently assumes the source data already contains the declared sentinel value.
...
- `encode_lerc2_one_sweep_bands_with_no_data` writes v6 no-data metadata for bands flagged by the caller. It currently preserves values already equal to the supplied no-data sentinel and does not yet implement the full C++ sentinel replacement heuristic.
...
- Encode-side C ABI functions now route constant, one-sweep single-band, one-sweep multi-band, and 4D inputs with or without active no-data metadata through safe Rust encoders. Active no-data encode currently requires the caller's data to already contain the declared no-data sentinel.
...
5. Add encode paths after decode parity is established.
   ...
   - Version 6 no-data metadata one-sweep encoding is now available as a safe Rust helper when data already contains the sentinel value.
...
6. Add FFI surface.
   ...
   - Constant, one-sweep single-band, one-sweep multi-band, and 4D C ABI encode paths now use the safe Rust encoder for no-data-free data and metadata-only no-data sentinels.
